...
export function initLiveCall(onReason){
 ...
 const streamingReady=()=>capabilities?.asr===true&&capabilities?.asrBackend!=='openai-whisper';
 ...
 async function connect(context,source,stream,audible=false){
  ...
  const chunker=new AudioChunker(context.sampleRate,accept);await context.audioWorklet.addModule('/pcm-worklet.js');
  ...
  const current={context,source,stream,processor,gain,chunker};graph=current;source.connect(processor);processor.connect(gain);gain.connect(context.destination);
  processor.port.onmessage=e=>{if(e.data.flushed){current.flushDone?.();return;}if(token!==generation||!session||!e.data.pcm)return;
   $('live-level').value=Math.min(1,rms(e.data.pcm)*8);chunker.push(e.data.pcm);
 ...
 async function start(kind){
  if(!streamingReady()){status(capabilities?.asrBackend==='openai-whisper'?'The configured Whisper medium CPU backend is batch-only. Use a short recorded clip or guided demo instead of continuous capture.':capabilities?.reasons?.asr||'Local ASR is unavailable. Capture was not started.');return;}
  if(starting||busy||stopping||session?.active)return;
  generation++;const token=generation;session={source:kind,mode:'live-asr',startedAt:new Date().toISOString(),active:false,entries:[],events:[],gaps:0,peak:null};queue=[];starting=true;render();renderRisk();controls();status('Opening audio source…');
  let stream,context;
  try{
   ...
   session.active=true;starting=false;controls();status(`${kind} · listening. Audio is transcribed in 3–8 second segments; video is neither processed nor uploaded.`);
   $('pipeline-input').textContent='Capturing audio';$('pipeline-text').textContent='Local ASR';$('pipeline-analysis').textContent='Waiting for speech';
 ...
 return {reset,setCapabilities(value){
   ...
   $('live-capability').textContent=value?.asr
     ?value.asrBackend==='openai-whisper'
       ?'Actual local batch ASR is available, but continuous capture is disabled because this CPU checkpoint is slower than real time. Use a recorded clip or the audio-only recognition button.'
       :'Actual local speech recognition. Browser permission is required; no speaker separation.'
     :value?.reasons?.asr||'Local ASR unavailable. Live capture is disabled; guided demos and text rules remain usable.';
